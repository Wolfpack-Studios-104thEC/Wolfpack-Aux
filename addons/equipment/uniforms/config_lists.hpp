

#define ITEM_LIST \
    QGUNIFORM(Phase_2_Uniform_CT), \
    QGUNIFORM(Phase_2_Uniform_VCT), \
    QGUNIFORM(Phase_2_Uniform_CSP1), \
    QGUNIFORM(Phase_2_Uniform_CSP2), \
    QGUNIFORM(Phase_2_Uniform_Medic_CSP1), \
    QGUNIFORM(Phase_2_Uniform_Medic_CSP2), \
    QGUNIFORM(Phase_2_Command_Uniform_Base), \
    QGUNIFORM(Phase_2_ARC_Uniform_Base), \
    QGUNIFORM(Phase_2_Uniform_Base_Halloween), \

#define UNIT_LIST \
    QGUNIT(Phase_2_Unit_Trooper), \
    QGUNIT(Phase_2_Unit_VCT), \
    QGUNIT(Phase_2_Unit_CSP1), \
    QGUNIT(Phase_2_Unit_CSP2), \
    QGUNIT(Phase_2_Unit_Medic_CSP1), \
    QGUNIT(Phase_2_Unit_Medic_CSP2), \
    QGUNIT(Phase_2_Command_Unit_Base), \
    QGUNIT(Phase_2_ARC_Unit_Base), \
    QGUNIT(Phase_2_Unit_Base_Halloween), \
